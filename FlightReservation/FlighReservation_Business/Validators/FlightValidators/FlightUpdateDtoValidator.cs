...
namespace FlighReservation_Business.Validators.FlightValidators
{
    public class FlightUpdateDtoValidator: AbstractValidator<FlightUpdateDto>
    {
        public FlightUpdateDtoValidator()
        {
            ...
            RuleFor(x => x.AdminComment).MaximumLength(500).WithMessage("Admin comment must not exceed 500 characters.");
            RuleFor(x => x.Status).IsInEnum().WithMessage("Invalid flight status.");
